...
5. Never generate <information>; it is returned by the retriever.
6. If evidence is sufficient, finish with <answer>...</answer>.
7. The final answer must be a short answer, not an explanation."""


FIRST_TURN_PROMPT = """Generate two complementary financial search queries for the question.

Output exactly this structure:
<think>identify the required financial quantities</think>
...
For a ratio or calculation, query different required quantities such as numerator and denominator.
Question: {question}"""


NEXT_TURN_PROMPT = """Continue the financial trajectory using only the retrieved information below.
...
<think>show the short calculation from retrieved values</think>
...
def gold_evidence_hit(information, gold_evidence):
    info_tokens = set(re.findall(r"[a-z0-9.%-]+", information.lower()))
    substantive_hits = 0
    substantive_total = 0
    for evidence in gold_evidence:
        evidence_tokens = set(re.findall(r"[a-z0-9.%-]+", evidence["text"].lower()))
        if len(evidence_tokens) < 3:
            continue
        substantive_total += 1
        if len(evidence_tokens & info_tokens) / len(evidence_tokens) >= 0.8:
            substantive_hits += 1
    required_hits = min(2, substantive_total)
    return substantive_total > 0 and substantive_hits >= required_hits
...
def retrieve(args, queries, report_id):
...
def make_record(example, assistant_content, queries_by_turn, information_blocks, final_answer, warnings):
    ...
    hit = gold_evidence_hit(info_text, example.get("gold_evidence", []))
...
def build_one(args, example):
    ...
    for turn_idx in range(args.max_turns):
        ...
        results = retrieve(args, turn_queries, example["report_id"])
...
def main():
    ...
    parser.add_argument("--data_source", default="finqa")
    parser.add_argument("--split", default="train")
    parser.add_argument("--output", default="data/finance_finqa/sft/finqa_sft_20.jsonl")
    parser.add_argument("--rejected_output", default="data/finance_finqa/sft/finqa_sft_20_rejected.jsonl")
    parser.add_argument("--report_output", default="data/finance_finqa/sft/finqa_sft_20_report.json")
